...
public class Main {
    ...
    private static void printHelpPanel() {
        System.console().format("> select ~ selecting a column from a specific table or all columns %nand it can be " +
        ...
        while (true) {
            ...
            while (true) {
                if (inputQuery.compareTo("") == 0) {
                    inputQuery = console.readLine(">> ").trim();
                    if (inputQuery.toLowerCase().compareTo(".help") == 0 || inputQuery.toLowerCase().compareTo(".quit") == 0 ) {
                        break;
                    }
                    ...
                    inputQuery = inputQuery.concat(" " + console.readLine().trim());
