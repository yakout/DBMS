...
public class Main {
    ...
    private static void printHelpPanel() {
        System.console()
                .format("> select ~ selecting a column from a specific table "
                        + "or all columns %nand it can be "
                        + "under some conditions using the where keyword.%n"
                        + "> create ~ creating a database or creating a table"
                        + " %nwith specified columns' "
                        + "attributes and data types.%n"
                        + "> drop ~ dropping or deleting a database or a "
                        + "table.%n"
                        + "> insert into ~ inserting specific values into "
                        + "specific columns in a table.%n"
                        + "> delete ~ deleting all data stored in a certain "
                        + "table %nor specific data stored "
                        + "in certain columns using the where keyword.%n"
                        + "> update ~ updating all table's columns or "
                        + "updating specific columns %nunder some conditions "
                        + "using "
                        + "the where keyword.%n");
    }
            ...
            if (inputQuery.compareTo("") == 0) {
                ...
                if (input.compareTo("%n") != 0) {
                    ...
                    if (inputQuery.toLowerCase().compareTo(".help") == 0
                            || inputQuery.toLowerCase().compareTo(".quit") ==
                            0) {
                        break;
                    }
